      ...
      if (itemStat.isSymbolicLink()) {
        ...
        if (linkStat.isDirectory()) {
          ...
          if (fs.existsSync(packagePath)) {
            ...
            // Search for any dependencies of the package. They should be included to metro so the packages get
            // imported in the bundle.
            for (let dependency in packageInfo.dependencies) {
              const packageId = dependency;
              moduleMappings[packageId] = path.join(linkPath, 'node_modules', dependency);
            }
          }
        }
...
console.log('Excluding following dev dependencies:');
console.log(moduleExclusions);
...
const config = {
  ...
  resolver: {
    ...
    nodeModulesPaths: Object.values(moduleMappings),
  },
